...
def setup(args):
    ...
    utils.create_dirs([config['save_dir'], config['pred_dir'], config['LLM'], config['tensorboard_dir']])
...
def evaluate(evaluator):
    ...
    with torch.no_grad():
        aggr_metrics, per_batch, init_states = evaluator.evaluate(epoch_num=None, keep_all=True)
...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    eval_start_time = time.time()
    with torch.no_grad():
        aggr_metrics, per_batch, hidden_state, ts_enc = val_evaluator.evaluate(epoch, keep_all=True)
    eval_runtime = time.time() - eval_start_time
    ...
    if condition:
        ...
        np.save('hidden_state.npy', hidden_state)
        np.save('init_ts_state.npy', ts_enc)
...
class SupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        for i, batch in enumerate(self.dataloader):
            ...
            padding_masks = padding_masks.to(self.device)  # 0s: ignore
            # regression: (batch_size, num_labels); classification: (batch_size, num_classes) of logits
            predictions, hidden_state, init_states = self.model(X.to(self.device), padding_masks)
            loss = self.loss_module(predictions, targets)  # (batch_size,) loss for each sample in the batch
            batch_loss = torch.sum(loss).cpu().item()
            ...
            per_batch['metrics'].append([loss.cpu().numpy()])
            per_batch['IDs'].append(IDs)
            ts_enc.append(init_states.detach().cpu().numpy())
        ...
        ts_enc = np.concatenate(ts_enc, axis=1)
        ...
        if keep_all:
            return self.epoch_metrics, per_batch, hidden_states, ts_enc
        else:
            return self.epoch_metrics
