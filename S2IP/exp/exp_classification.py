...
class Exp_Classification(Exp_Basic):
    ...
    def _build_model(self):
        ...
        # model init
        model = self.model_dict[self.args.model].Model(self.args, DEVICE).float()
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, label, mean, stdev) in enumerate(vali_loader):
                ...
                loss = self.criterion(pred, label.long().squeeze(-1).cpu())
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label, mean, stdev) in tqdm(enumerate(train_loader)):
                iter_count += 1
            ...
            print(
                "Epoch: {0}, Steps: {1} | Train Loss: {2:.3f} Test Loss: {3:.3f} Test Acc: {4:.3f}"
                .format(epoch + 1, train_steps, train_loss, test_loss, test_accuracy))
    ...
    def test(self, setting):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, label, mean, stdev) in enumerate(test_loader):
                batch_x = batch_x.float().to(self.device)
                # padding_mask = padding_mask.float().to(self.device)
                label = label.to(self.device)
                outputs, _ = self.model(batch_x, mean, stdev)
                preds.append(outputs.detach())
                trues.append(label)
        ...
        file_name=f'result_classification_{self.args.LLM}_{self.args.model_id}.txt'
