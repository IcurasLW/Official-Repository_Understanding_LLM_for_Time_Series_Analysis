...
class Exp_Imputation(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, means, stdev, imput_mask) in tqdm(enumerate(train_loader)):
                iter_count += 1
                model_optim.zero_grad()
                batch_x = batch_x.float().to(self.device)

                outputs, res = self.model(batch_x, means, stdev, imput_mask)
                batch_x = batch_x.sum(2)
                loss = criterion(outputs[imput_mask == 0], batch_x[imput_mask == 0])
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, means, stdev, imput_mask) in enumerate(vali_loader):
                batch_x = batch_x.float().to(self.device)
                outputs, res_loss = self.model(batch_x, means, stdev, imput_mask)
                pred = outputs.detach().cpu()
                batch_x = batch_x.sum(2)
                true = batch_x.detach().cpu()
                imput_mask = imput_mask.detach().cpu()
                
                loss = criterion(pred[imput_mask == 0], true[imput_mask == 0])
    ...
    def test(self, setting, test=0):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, means, stdev, imput_mask) in enumerate(test_loader):
                batch_x = batch_x.float().to(self.device)
                # imputation
                outputs, res_loss = self.model(batch_x, means, stdev, imput_mask)
                ...
                preds.append(pred)
                trues.append(true)
                masks.append(imput_mask.detach().cpu())
        ...
        f = open(f"result_imputation_{self.args.LLM}_{self.args.model_id}.txt", 'a')
        f.write(setting + "  \n")
