    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                hidden_states = torch.concatenate(outputs['intermidiate_time'], dim=0).unsqueeze(0).detach().cpu().numpy() # B, I, S, E
                time_tokens.append(hidden_states)
                preds.append(pred)
                trues.append(true)
                if i % 20 == 0:
        ...
        time_tokens = np.concatenate(time_tokens, axis=0)
        ...
        f.write('\n')
        f.close()
        np.save(folder_path + 'time_tokens.npy', time_tokens, allow_pickle=True)
