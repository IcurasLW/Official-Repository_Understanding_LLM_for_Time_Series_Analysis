...
from layers.Embed import PatchEmbedding, PatchEmbedding_classification
import transformers
...
class FlattenHead(nn.Module):
    def __init__(self, n_vars, nf, n_classes, head_dropout=0):
        super().__init__()
        self.n_vars = n_vars
        self.flatten = nn.Flatten(start_dim=-2)
        self.linear = nn.Linear(nf, n_classes)
...
class Model(nn.Module):
    ...
    def __init__(self, configs, n_classes, d_inp):
        ...
        self.normalize_layers = Normalize(d_inp, affine=False)
        self.n_classes = n_classes
        self.configs = configs
        
        ###### Select LLM if backbone is not linear, Att, Trans ######
        if self.configs.llm_model in ['Linear', 'Att', 'Trans', 'NoLLM']:
            self.backbone = self.get_llm(configs)
        ...
        self.patch_embedding = PatchEmbedding_classification(configs.d_model, self.patch_len, self.stride, configs.dropout, d_inp)
        ...
        # self.output_projection = FlattenHead(configs.enc_in, 217*self.d_llm, n_classes, head_dropout=configs.dropout)
        self.output_projection = nn.Linear(in_features=self.d_llm, out_features=self.n_classes)


    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_de):
        ...
        for b in range(x_enc.shape[0]):
            ...
            prompt_ = (
                ...
                f"the variable trend of input is {['upward' if each > 0 else 'downward' for each in trends[b]]}, "
        ...
        prompt = self.tokenizer(prompt, return_tensors="pt", padding=True, truncation=True, max_length=1024 - x_enc.shape[1]).input_ids
        prompt_embeddings = self.llm_model.get_input_embeddings()(prompt.to(DEVICE))  # (batch, prompt_token, dim)
        source_embeddings = self.mapping_layer(self.word_embeddings.permute(1, 0)).permute(1, 0)
        
        enc_out = self.reprogramming_layer(enc_out, source_embeddings, source_embeddings)
        llama_enc_out = torch.cat([enc_out, prompt_embeddings], dim=1)
        
        if self.configs.llm_model in ['Random', 'GPT2']:
            dec_out = self.llm_model(inputs_embeds=llama_enc_out).last_hidden_state
        else:
            dec_out = self.backbone(inputs_embeds=llama_enc_out).last_hidden_state
    ...
    def get_llm(self, config):
        ...
        if config.llm_model not in ['Linear', 'Att', 'Trans', 'NoLLM']:
            ...
            if config.llm_model == 'Linear':
                self.backbone = LinearEncoder(config)
            elif config.llm_model == 'Att':
                self.backbone = MHAtt_Backbone(config)
            elif config.llm_model == 'Trans':
                self.backbone = Trans_Backbone(config)
