...
parser.add_argument('--anomaly_ratio', type=float, default=2, help='prior anomaly ratio (%)')
...
logging.basicConfig(
                    ...
                    filename=f'{args.task_name}_{args.llm_model}_{args.model_id}.log',  # Set the log file path
...
accelerator.print(f'============================================ Task: {args.task_name}_{args.data}_{args.pred_len} ============================================')
...
for ii in range(args.itr):
    ...
    for epoch in range(args.train_epochs):
        ...
        vali_loss = vali_anomaly_multi_gpu(args, model, vali_data, vali_loader, criterion, mae_metric, accelerator)
        test_acc, test_prec, test_recall, test_f1 = test_anomaly_multi_gpu(args, model, path, accelerator)
        val_msg = "Epoch: {0} | Train Loss: {1:.7f} Vali Loss: {2:.7f}".format(epoch + 1, train_loss, vali_loss)
        test_msg = f"Test reults: Accuracy: {test_acc}, Precision: {test_prec} Recall: {test_recall}  F1: {test_f1}"
